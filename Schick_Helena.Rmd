...
Our key results for the success metrics are an accuracy and precision of more than 80%.
Our project is deemed a failure, if the accuracy and precision are lower than 80%.
...
df_train %>%
  ...
  geom_boxplot()
```
Startups having more than 20 relationships are outliers and are removed.

```{r}
# remove relationships > 20
train_data <- subset(train_data, relationships < 20)
```
...
train_data <- subset(train_data, funding_total_usd < 100000000)
```
...
train_data <- subset(train_data, funding_rounds < 6)
```
...
train_data <- subset(train_data, founded_at > "1995-01-01")
```

## Feature Engineering

Feature engineering is the process of using domain knowledge to extract meaningful features from the data. The goal of this process is to keep those features that improve the predictions from our model.
For this a pipeline is created that included the standardization of numeric features and one hot encoding of catgeorical features. As the categories and states are already one hot encoded, their categorical featured need to be removed.
As the anomalies, like outliers, missing values or wrong data types, have already been fixed, no further feature transformations are needed in this step.

```{r}
# remove category and stat categorical feature
train_data <- train_data %>%
  select(-state_code, -category_code)
...
```{r}
# remove highly correlated location information, only keeping one hot encoded state
train_data <- train_data %>%
  select(-longitude, -latitude, -zip_code, -city)
...
```{r}
# convert all date variables to numeric (could not make it work in the recipe)
train_data$founded_at <- 
  as.numeric(train_data$founded_at)
train_data$first_funding_at <- 
  as.numeric(train_data$first_funding_at)
train_data$last_funding_at <- 
  as.numeric(train_data$last_funding_at)

test_data$founded_at <- 
  as.numeric(test_data$founded_at)
test_data$first_funding_at <- 
  as.numeric(test_data$first_funding_at)
test_data$last_funding_at <- 
  as.numeric(test_data$last_funding_at)
```

```{r}
# remove name and id, as they are unique and thus do not serve the model
train_data <- train_data %>%
  select(-name, -id)
...
As the data is pre-categorized, having status as the to be predicted label, supervised learning in form of classification is used.
As the data is not too complex, neural networks are not needed. Instead, two common classification algorithms are compared:
- Logistic Regression: Predicts a dependent variable by analyzing the relationship between one or more existing independent variables.
- Decision Trees: Uses tree-like model of decisions and their possible consequences. examples (K) closest to the query, then votes for the most frequent label.
...
At first, we need to decide which features we want to include in our model.
As identified in the correlation matrix, there is a strong positive linear correlation between the first and last funding and milestone age of startups. These highly correlated features are removed and this is included in the recipe of Feature Engineering already.

```{r}
# create recipe
df_rec <-
  recipe(status ~ ., data = train_data) %>%
  step_naomit(everything(), skip = TRUE) %>% #removes rows with NaN values
...
The accuracy rates the overall correct classifications. Precision describes how much of the samples, which have been classified as positive are actual positive. And recall describes how much of the true positive samples has been classified as positive.

The logistic regression model has an accuracy of 74%. 77% of the startups that it predicted to be successful, are actually successful and 89% of true positive samples habe been classified as positive.
...
A confusion matrix is created to compare the amounts of predictions.
...
```
325 startups are True Positives, as they have been predicted to be acquired and have also been acquired in reality. On the other hand, 61 startups that closed have also been predicted to close, those are the True Negatives. There are 38 False Negatives, that have been acquired, but were predicted to close. The fourth kind are the False Positives, those have closed in reality, but were predicted to be acquired. There are 94 False Positives in this logistic regression and this is the number that should be as low as possible.
...
xgb_res %>% 
  collect_metrics(summarize = TRUE)
```
The Gradient Boosted Tree has an accuracy of 73%, recall of 88% and most importantly a precision of 75%.
...
log_pred %>%
  ...
  autoplot(type = "heatmap")
```
There are 116 False Positives, so startups thatt have been predicted to be acquired, but actually closed.
...
```
The accuracy and precision of the Logistic Regression are a little higher and thus, this is the model with the best performance. The objective for a good model, is supposed to be 80% for accuracy and precision and those are not quite reached, but at it is close.

## Evaluate Model

The best performing model - Logistic Regression - is now evaluated on test data.
