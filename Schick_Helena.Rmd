...
age_first_funding_year and age_last_funding_year and age_first_milestone_year and age_last_milestone_date have negative minima --> how many? delete?
founded_at in 20th century
...
```
No outliers, many that have a their first or last funding after 5 years.
...
```{r}
# histogram first_funding_at
df_train %>%
  ggplot(aes(x = first_funding_at)) +
  geom_histogram()
```

```{r}
# histogram last_funding_at
df_train %>%
  ggplot(aes(x = last_funding_at)) +
  geom_histogram()
```
