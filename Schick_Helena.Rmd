...
log_res <- 
  ...
  fit_resamples(
    resamples = cv_folds, 
    metrics = metric_set(yardstick::recall, yardstick::precision, f_meas, accuracy, kap, roc_auc, sens, yardstick::spec),
...
```
The next step is a plot of the predicted probability distributions for the two classes.
```{r}
log_res %>%
  collect_predictions() %>% 
  ggplot() +
  geom_density(aes(x = .pred_acquired, fill = status), 
               alpha = 0.5)
```
...
model_comp %>% 
  ...
  scale_fill_brewer(palette = "Blues") +
   geom_text(
     size = 3,
     aes(label = round(mean_accuracy, 2), y = mean_f_meas + 0.08),
     vjust = 1
  )
...
model_comp %>% 
  ...
  scale_fill_brewer(palette = "Blues") + 
     geom_text(
     size = 3,
     aes(label = round(mean_precision, 2), y = mean_roc_auc + 0.08),
     vjust = 1
  )
```
